...
class Estate(models.Model):
    ...
    date_availability = fields.Date(
        copy=False, default=lambda self: self._default_date_availability()
    )
    ...
    saleman_id = fields.Many2one(
        "res.users", copy=False, default=lambda self: self.env.user
    )
    ...
    def _check_price(self):
        for record in self:
            ...
            if (
                ...
                and float_compare(
                    record.selling_price, lowest_accepted_price, precision_digits=2
                )
                == -1
            ):
    ...
    def action_sell(self):
        for record in self:
            ...
            if not any(offer.state == "accepted" for offer in record.offer_ids):
                raise UserError(
                    "You can't sell a property that doesn't have an accepted offer."
                )
